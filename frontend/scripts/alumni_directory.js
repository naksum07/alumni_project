...
(function () {
  ...
  function renderCurrentPage() {
      ...
      if (alumniGrid) {
          alumniGrid.innerHTML = pageData.map(a => {
              const fullName = escapeHTML(a.full_name || 'Alumni Member');
              const initial = fullName.charAt(0).toUpperCase();
              const avatarHtml = a.profile_picture
                ? `<img src="${escapeHTML(a.profile_picture)}" class="w-20 h-20 mx-auto rounded-full object-cover border-2 border-blue-200 shadow-sm" alt="${fullName}">`
              ...
              const linkedin = safeUrl(a.linkedin_url);
              ...
              return `
                <div class="alumni-card bg-white rounded-xl shadow-md p-6 text-center border border-gray-100 hover:-translate-y-1 hover:shadow-lg transition flex flex-col justify-between">
                  <div>
                    ...
                    ${a.city ? `<p class="text-gray-500 text-xs mt-1 font-medium">📍 ${escapeHTML(a.city)}</p>` : ''}
                    ${linkedin ? `
                      <div class="mt-3">
                        <a href="${linkedin}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-1.5 text-xs text-blue-700 hover:text-blue-900 font-semibold bg-blue-50 hover:bg-blue-100 px-3 py-1.5 rounded-full transition">
                ...
                <a href="my-profile.html?id=${encodeURIComponent(a.id)}" class="block mt-5 bg-blue-700 text-white py-2.5 rounded-lg hover:bg-blue-800 font-medium transition">
                  View Profile
                </a>
              </div>
              `;
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', loadLiveAlumni);
  } else {
    loadLiveAlumni();
  }
