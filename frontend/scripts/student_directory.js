  ...
  function renderCurrentPage() {
    ...
    if (studentGrid) {
      studentGrid.innerHTML = pageData.map(s => {
        const fullName = escapeHTML(s.full_name || 'Student Member');
        const initial = fullName.charAt(0).toUpperCase() || '?';
        const avatarHtml = s.profile_picture
          ? `<img src="${escapeHTML(s.profile_picture)}" class="w-20 h-20 mx-auto rounded-full object-cover border-2 border-emerald-200 shadow-sm" alt="${fullName}">`
          : `<div class="w-20 h-20 mx-auto rounded-full flex items-center justify-center bg-emerald-100 text-emerald-700 text-3xl font-bold border-2 border-emerald-200 shadow-sm">${initial}</div>`;
        const linkedin = safeUrl(s.linkedin_url);
        ...
        return `
          <div class="student-card bg-white rounded-xl shadow-md p-6 text-center border border-gray-100 hover:-translate-y-1 hover:shadow-lg transition flex flex-col justify-between">
            <div>
              ...
              <p class="text-gray-500 text-xs mt-1">${escapeHTML(s.email || '-')}</p>
              ...
              ${s.enrollment_number ? `<p class="text-gray-500 text-xs font-mono mt-1">Roll No: ${escapeHTML(s.enrollment_number)}</p>` : ''}
              <p class="text-gray-500 text-sm mt-2">${escapeHTML(s.department || 'Department')} ${s.graduation_year ? `• Batch ${escapeHTML(String(s.graduation_year))}` : ''}</p>
              ${s.city ? `<p class="text-gray-500 text-xs mt-1 font-medium">📍 ${escapeHTML(s.city)}</p>` : ''}
              ${linkedin ? `
                <div class="mt-3">
                  <a href="${linkedin}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center justify-center gap-1.5 text-xs text-emerald-700 hover:text-emerald-900 font-semibold bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-full transition">
            ...
            <a href="my-profile.html?id=${encodeURIComponent(s.id)}" class="block mt-5 bg-blue-700 hover:bg-blue-800 text-white py-2.5 rounded-lg font-medium transition">
              View Profile
            </a>
          </div>
        `;
